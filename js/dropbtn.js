// $function(){
//     $(".all-p").click(function(){
//         $(".all-price").css("display","block");
//         $('.first-price').css("display","none");
//         $('.second-price').css("display","none");
//         $('third-price').css("display","none");
//         $('fourth-price').css("display","none");
//     });
//     $(".first-p").click(function(){
//         $(".first-price").css("display","block");
//         $('.all-price').css("display","none");
//         $('.second-price').css("display","none");
//         $('third-price').css("display","none");
//         $('fourth-price').css("display","none");
//     });
// }



$(".all-p").click(function(){
    $(".all-price").css("display","block");
    $(".all-price").css("font-weight","bold");
    $(".first-price").css("display","none");
    $(".first-price").css("font-weight","400");
    $(".second-price").css("display","none");
    $(".second-price").css("font-weight","400");
    $(".third-price").css("display","none");
    $(".third-price").css("font-weight","400");
    $(".fourth-price").css("display","none");
    $(".fourth-price").css("font-weight","400");
});
$(".first-p").click(function(){
    $(".first-price").css("display","block");
    $(".first-price").css("font-weight","bold");
    $(".all-price").css("display","none");
    $(".all-price").css("font-weight","400");
    $(".second-price").css("display","none");
    $(".second-price").css("font-weight","400");
    $(".third-price").css("display","none");
    $(".third-price").css("font-weight","400");
    $(".fourth-price").css("display","none");
});
$(".second-p").click(function(){
    $(".second-price").css("display","block");
    $(".second-price").css("font-weight","bold");
    $(".all-price").css("display","none");
    $(".all-price").css("font-weight","400");
    $(".first-price").css("display","none");
    $(".first-price").css("font-weight","400");
    $(".third-price").css("display","none");
    $(".third-price").css("font-weight","400");
    $(".fourth-price").css("display","none");
    $(".fourth-price").css("font-weight","400");
});
$(".third-p").click(function(){
    $(".third-price").css("display","block");
    $(".third-price").css("font-weight","bold");
    $(".all-price").css("display","none");
    $(".all-price").css("font-weight","400");
    $(".second-price").css("display","none");
    $(".second-price").css("font-weight","400");
    $(".first-price").css("display","none")
    $(".fourth-price").css("display","none");
});
$(".fourth-p").click(function(){
    $(".fourth-price").css("display","block");
    $(".fourth-price").css("font-weight","bold");
    $(".all-price").css("display","none");
    $(".all-price").css("font-weight","400");
    $(".second-price").css("display","none");
    $(".second-price").css("font-weight","400");
    $(".first-price").css("display","none");
    $(".first-price").css("font-weight","400");
    $(".third-price").css("display","none");
    $(".third-price").css("font-weight","400");
});


// $(".function-1").mouseover(function(){
//     $(".overlay-1").hide();
//     });
//     $(".function-1").mouseleave(function(){
//         $(".overlay-1").fadeIn(1000);
//     });

//     $(".function-2").mouseover(function(){
//     $(".overlay-2").hide();
//     });
//     $(".function-2").mouseleave(function(){
//         $(".overlay-2").fadeIn(1000);
//     });

//     $(".function-3").mouseover(function(){
//     $(".overlay-3").hide();
//     });
//     $(".function-3").mouseleave(function(){
//         $(".overlay-3").fadeIn(1000);
//     });